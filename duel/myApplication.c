                   ...
                   DDS_Long sleep_time, DDS_Long count)
{
    ...
    *DDS_StringSeq_get_reference(&dp_qos.discovery.initial_peers,0) = DDS_String_dup(peer);
    ...
    application->participant =
        DDS_DomainParticipantFactory_create_participant(factory, pub_domain_id,
                                                        &dp_qos, NULL,
    ...
    sprintf(application->pub_topic_name, "Example HelloWorld1");
    ...
    sprintf(application->sub_topic_name, "Example HelloWorld2");
...
{
    DDS_Entity *pub_entity, *sub_entity;
    DDS_ReturnCode_t retcode;

    pub_entity = DDS_DomainParticipant_as_entity(application->participant);
    retcode = DDS_Entity_enable(pub_entity);
